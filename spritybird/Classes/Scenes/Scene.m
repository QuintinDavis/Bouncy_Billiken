...
// Obstacles
#define VERTICAL_GAP_SIZE 140
#define FIRST_OBSTACLE_PADDING 40
...
- (void) createScore
{
    ...
    scoreLabel.position = CGPointMake(20, CGRectGetMaxY(self.frame)- 30);
    scoreLabel.alpha = 1;
        ...
        if(X(topPipe) + WIDTH(topPipe)/2 > bird.position.x &&
           X(topPipe) + WIDTH(topPipe)/2 < bird.position.x + FLOOR_SCROLLING_SPEED){
            ...
            if(self.score>=10){
                ...
                scoreLabel.position = CGPointMake(CGRectGetMidX(self.frame), 25);
            }
        }
    }
}
